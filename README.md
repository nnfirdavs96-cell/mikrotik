...
### 🔜 Этап 3 — что осталось (план)

- Автоматический **captive redirect** (MikroTik hotspot или DNS-перехват) — сейчас портал открывается вручную/по правилу firewall.
- **Несколько активных MikroTik** одновременно и несколько guest-сетей (схема БД уже поддерживает несколько устройств; нужна логика выбора устройства per-client и UI).
- **Личный кабинет клиента** (история, продление тарифа без повторной регистрации).
- **Шифрование пароля MikroTik** в БД (сейчас хранится как есть — см. «Рекомендации по безопасности»).
- Экспорт отчётов (CSV/Excel) по платежам и клиентам.
...
2. Пароль MikroTik не хранится в коде — только в БД/`.env`.
